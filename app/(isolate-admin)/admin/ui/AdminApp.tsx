...
export default function AdminApp() {
  ...
  // --- NEW: load seasons and auto-fill seasonId ---
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const ids: string[] = (j?.seasons ?? []).map((s: any) => s.id);
        setSeasons(ids);
        if (!seasonId && ids.length) setSeasonId(ids[0]); // default to first season
      } catch {
        // ignore
      }
    })();
          ...
          <div className="grid gap-4 max-w-xl">
            <div className="border rounded p-4">
              ...
              <div className="flex gap-2 mb-2">
                {/* Use a select so seasonId is always valid */}
                <select
                  value={seasonId}
              ...
              <div className="flex items-center gap-3">
                ...
                {/* quick helper link to see valid GWs for the selected season */}
                {seasonId && (
                  <a
                    className="text-sm text-slate-600 underline"
                    href={`/api/admin/seasons/${seasonId}/gameweeks`}
                    target="_blank" rel="noreferrer"
                  >
                    View valid GW numbers
                  </a>
                )}
              </div>
            </div>

            {/* Link to server-rendered reports UI */}
            <div className="border rounded p-4">
              <h3 className="font-medium mb-2">New: Charts &amp; Eliminations</h3>
              <p className="text-sm text-gray-600 mb-3">
                Open the server-powered reporting UI that generates and displays pie charts and the eliminated list.
              </p>
              <Link
  ...
  }, [gwNumber, processResults, seasonId, seasons, section, seedClubs, syncFixtures]);
