...
export async function POST(req: Request) {
  try {
    ...
    if (!viewer?.isAdmin) return NextResponse.json({ ok: false, error: "Unauthorized" }, { status: 401 });

    const body = await req.json().catch(() => ({}));
    const seasonId = (body.seasonId ?? "").trim();
    const gwNumber = Number(body.gwNumber);
    if (!seasonId || Number.isNaN(gwNumber)) return NextResponse.json({ ok: false, error: "seasonId/gwNumber required" }, { status: 400 });

    const eff = await effectiveDeadline(seasonId, gwNumber);
